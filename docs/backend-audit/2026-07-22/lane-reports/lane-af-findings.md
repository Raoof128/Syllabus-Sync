...
- `lib/security/csrf.ts:267` (`getTrustedOrigins()`) adds `https://maps.googleapis.com` as a CSRF-trusted origin for inbound POSTs. There is no mechanism by which a browser would ever send `Origin: https://maps.googleapis.com` on a request to this app (Origin reflects the _page's_ origin, not a resource it loads), so this entry is currently inert. It doesn't create a live bypass, but it's an unnecessary widening of the trust set that should be removed or documented.
