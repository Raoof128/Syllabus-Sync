...
describe('TOTP Enroll Cache-Control (Security Audit)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockMfaEnrollLimiter.mockResolvedValue({ allowed: true, remaining: 9, resetIn: 3600 });
  });
  ...
  it('returns 429 when rate limited', async () => {
    mockMfaEnrollLimiter.mockResolvedValue({ allowed: false, remaining: 0, resetIn: 600 });
