...
async function logComputedStylesForViolations(page: Page, results: AxeResults, testInfo: TestInfo) {
    ...
    return await page.evaluate(
      ({ sel }: { sel: string }) => {
        ...
        try {
          ...
          const descendants = Array.from(visible.querySelectorAll('*'))
            .filter((el: Element) => {
              try {
  ...
  const rootVars = await page.evaluate(() => ({
    ...
    mqContentSecondary: getComputedStyle(document.documentElement).getPropertyValue(
      '--mq-content-secondary',
    ),
    cContentSecondary: getComputedStyle(document.documentElement).getPropertyValue(
      '--c-content-secondary',
    ),
        ...
        try {
          ...
          if (matched && matched.debugAttr) {
            ...
            try {
              ...
              await page
                .locator(`[${matched.debugAttr}='1']`)
                .first()
                .screenshot({ path: pngPath });
              info.screenshot = pngPath;
              // remove debug attribute
              await page.evaluate(
                ({ dbgAttr }: { dbgAttr: string }) => {
                  const n = document.querySelector(`[${dbgAttr}='1']`);
                  if (n) n.removeAttribute(dbgAttr);
                },
                { dbgAttr: matched.debugAttr },
              );
            } catch (err) {
