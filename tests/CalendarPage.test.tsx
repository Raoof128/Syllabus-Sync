...
vi.mock("@/lib/store/unitsStore", () => ({
  useUnitsStore: (selector: (state: typeof unitsState) => unknown) =>
    selector(unitsState),
}));

vi.mock("@/lib/store/eventsStore", () => ({
  useEventsStore: (selector: (state: typeof eventsState) => unknown) =>
    selector(eventsState),
}));

vi.mock("@/lib/store/todosStore", () => ({
  useTodosStore: (selector: (state: typeof todosState) => unknown) =>
    selector(todosState),
}));
...
describe("CalendarPage", () => {
  ...
  it("displays the Assignments section with pending count", async () => {
    ...
    const assignmentsHeading = screen.getByRole("heading", {
      name: new RegExp(
        `${translations.assignments}.*${translations.pending}`,
        "i",
      ),
    });
