...
const mocks = vi.hoisted(() => ({
  ...
  createAndSendVerificationMock: vi.fn(
    async (_admin: any, _userId: string, _email: string) => ({
      success: true,
    }),
  ),
...
describe("email resend verification API", () => {
  ...
  it("returns generic success when admin client is not configured", async () => {
    ...
    const { POST } =
      await import("@/app/api/auth/email/resend-verification/route");
  ...
  it("triggers resend when user exists and is not confirmed", async () => {
    const rpc = vi.fn().mockResolvedValue({
      data: [
        { user_id: "user-1", user_email: "user@example.com", user_meta: {} },
      ],
      error: null,
    });
