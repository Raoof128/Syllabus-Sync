...
describe("normaliseOverlayIds", () => {
  ...
  it("filters out invalid IDs", () => {
    expect(normaliseOverlayIds(["parking", "bogus", "exam"])).toEqual([
      "parking",
    ]);
  });
  ...
  it("removes duplicates", () => {
    expect(
      normaliseOverlayIds(["parking", "parking", "accessibility"]),
    ).toEqual(["parking", "accessibility"]);
  });

  it("orders by registry position regardless of input order", () => {
    expect(
      normaliseOverlayIds(["special_permits", "parking", "drinking_water"]),
    ).toEqual(["parking", "drinking_water", "special_permits"]);
  });
...
describe("URL helpers", () => {
  ...
  it("overlaysToURLParam returns stable ordered string", () => {
    expect(overlaysToURLParam(["accessibility", "parking"])).toBe(
