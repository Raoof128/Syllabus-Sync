...
export function LevelBadge({
  ...
  const { profile, loadProfile, hasLoaded, isLoading, getLevelTitle } =
    ...
    return (
      <div className={cn("flex items-center", config.container, className)}>
        <div
          className={cn(
            config.badge,
            "rounded-full bg-mq-background-secondary animate-pulse",
          )}
        />
        {showTitle && (
    ...
    <div className={cn("flex items-center", config.container, className)}>
      ...
      {showTitle && (
        <div className="flex flex-col">
          <span className={cn(config.title, "font-semibold text-mq-content")}>
            Lv. {level}
          </span>
