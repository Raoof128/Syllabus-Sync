...
export default function HomeKpiStrip() {
  ...
  const todayClassesCount = useMemo(() => {
    ...
    return units.reduce((count, unit) => {
      const classesToday =
        unit.schedule?.filter((s) => s.day === todayLabel) || [];
  ...
  const timeToDeadline = nextDeadline
    ? formatDistanceToNow(new Date(nextDeadline.dueDate), {
        addSuffix: true,
        locale: enAU,
      })
  ...
  const currentStress =
    stressConfig[stressLevel as keyof typeof stressConfig] || stressConfig.Low;
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {[1, 2, 3].map((i) => (
          <CardSolid
            key={i}
            className="p-4 h-24 animate-pulse flex items-center"
          >
  ...
  return (
    ...
    >
      ...
      <CardSolid className="p-4 flex items-center gap-4 relative overflow-hidden group">
        ...
        <div>
          <p className="text-sm text-mq-content-secondary font-medium">
            {t("classesToday")}
          </p>
      ...
      <CardSolid className="p-4 flex items-center gap-4 relative overflow-hidden group">
        ...
        <div className="min-w-0">
          ...
          {nextDeadline && (
            <p className="text-xs text-mq-content-tertiary truncate">
              {nextDeadline.title}
            </p>
          )}
        </div>
      ...
      <CardSolid className="p-4 flex items-center gap-4 relative overflow-hidden group">
        ...
        <div>
          ...
          <p className={`text-2xl font-bold ${currentStress.color}`}>
            {currentStress.label}
          </p>
        </div>
      </CardSolid>
