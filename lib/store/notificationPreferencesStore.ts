      ...
      (set, get) => ({
        ...
        initialize: async () => {
          ...
          const deadlinesEnabled =
            notificationService.isNotificationTypeEnabled("deadlines");
          const classesEnabled =
            notificationService.isNotificationTypeEnabled("classes");
          const eventsEnabled =
            notificationService.isNotificationTypeEnabled("events");
        ...
        scheduleDeadlineReminder: (
          ...
          const reminderTime = new Date(
            dueDate.getTime() - state.deadlineReminderTiming * 60 * 1000,
          );
          ...
          if (delay > 0) {
            ...
            const timeoutId = window.setTimeout(() => {
              ...
              set((s) => {
                const { [deadlineId]: _removed, ...rest } =
        ...
        scheduleClassReminder: (
          ...
          const now = new Date();
          const reminderTime = new Date(
            classTime.getTime() - state.classReminderTiming * 60 * 1000,
          );
          const delay = reminderTime.getTime() - now.getTime();
          ...
          if (delay > 0) {
            ...
            const timeoutId = window.setTimeout(() => {
              ...
              set((s) => {
                const { [reminderId]: _removed, ...rest } =
                  s.scheduledReminders;
                void _removed; // Silence unused variable warning
                return {
                  scheduledReminders: rest,
                  pendingReminders: Object.fromEntries(
                    Object.entries(s.pendingReminders).filter(
                      ([key]) => key !== reminderId,
                    ),
                  ),
                };
        ...
        scheduleEventReminder: (
          ...
          const now = new Date();
          const reminderTime = new Date(
            eventTime.getTime() - state.eventReminderTiming * 60 * 1000,
          );
          const delay = reminderTime.getTime() - now.getTime();
        ...
        cancelReminder: (id: string) => {
          ...
          if (state.scheduledReminders[id]) {
            ...
            set((s) => {
              ...
              void _removed; // Silence unused variable warning
              const { [id]: _removedPending, ...restPending } =
                s.pendingReminders;
        ...
        reschedulePending: () => {
          ...
          const state = get();
          if (!state.pushEnabled || state.permissionStatus !== "granted")
            return;
            ...
            const timeoutId = window.setTimeout(() => {
              if (reminder.type === "deadline") {
                ...
                const eventTime = new Date(
                  reminder.payload.eventTime as number,
                );
                const timeStr = eventTime.toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
              ...
              set((s) => {
                const { [id]: _removed, ...rest } = s.pendingReminders;
                void _removed;
                const { [id]: _timeout, ...restTimeouts } =
                  s.scheduledReminders;
                void _timeout;
                return {
