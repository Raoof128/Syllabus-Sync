...
type ThemePersistedState = Pick<
  ThemeState,
  "theme" | "resolvedTheme" | "glass"
>;
  ...
  persist(
    (set, get) => {
      return {
        ...
        setGlassMode: (mode) =>
          set((state) => ({ glass: { ...state.glass, mode } })),
        ...
        setGlassRefraction: (enabled) =>
          set((state) => ({
            glass: { ...state.glass, enableRefraction: enabled },
          })),
        setGlassSpringAnimations: (enabled) =>
          set((state) => ({
            glass: { ...state.glass, enableSpringAnimations: enabled },
          })),
        reset: () =>
          set({
            theme: "system",
...
export const useGlassIntensity = () =>
  useThemeStore((state) => state.glass.intensity);
