...
const normalizeUnit = (unit: Unit): Unit => ({
  ...unit,
  createdAt:
    unit.createdAt instanceof Date ? unit.createdAt : new Date(unit.createdAt),
      ...
      loadUnits: async () => {
        ...
        try {
          ...
          if (isAuthError) {
            ...
            console.warn(
              "Failed to load units from API, using persisted data:",
              error,
            );
            set({ hasLoaded: true });
          }
        ...
        try {
          ...
          set((state) => ({
            units: state.units.map((u) =>
              u.id === normalized.id ? serverNormalized : u,
            ),
          }));
      ...
      removeUnit: async (id) => {
        ...
        try {
          ...
          errorHandler.logError(
            error instanceof Error
              ? error
              : new Error(`Failed to remove unit ${id}`),
      ...
      updateUnit: async (id, updatedUnit) => {
        ...
        try {
          ...
          if (
            ...
            console.warn(
              `Unit ${id} not found on server, syncing state from database...`,
            );
            ...
            console.warn(
              "Found refreshedUnit:",
              refreshedUnit
                ? { id: refreshedUnit.id, code: refreshedUnit.code }
                : null,
            );
            ...
            if (refreshedUnit) {
              ...
              try {
                console.warn(
                  "Updating existing unit with ID:",
                  refreshedUnit.id,
                );
                const updated = await apiRequest<Unit>(
                  `/api/units/${refreshedUnit.id}`,
                  {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                ...
                set((state) => ({
                  units: state.units.map((u) =>
                    u.id === refreshedUnit.id ? normalized : u,
                  ),
                }));
                return normalized;
              } catch (retryError) {
                console.error(
                  "Failed to update unit after refresh:",
                  retryError,
                );
              ...
              try {
                ...
                set((state) => ({
                  units: [
                    ...state.units.filter(
                      (u) =>
                        u.code.toUpperCase() !==
                        optimisticUpdate.code.toUpperCase(),
                    ),
                    normalized,
                  ],
                }));
                ...
                if (
                  ...
                  console.warn(
                    "Unit already exists (409), refreshing to find it...",
                  );
                  await get().forceRefresh();
                  const searchCode = optimisticUpdate.code.toUpperCase();
                  const found = get().units.find(
                    (u) => u.code.toUpperCase() === searchCode,
                  );
                  if (found) {
                    console.warn(
                      "Found existing unit after 409:",
                      found.id,
                      found.code,
                    );
                    return found;
          ...
          errorHandler.logError(
            error instanceof Error
              ? error
              : new Error(`Failed to update unit ${id}`),
          ...
          if (state?.state?.units && Array.isArray(state.state.units)) {
            ...
            state.state.units = state.state.units.map((unit) => {
              ...
              if (unit.id && !isUUID(unit.id)) {
                ...
                const updatedUnit = {
                  ...unit,
                  id: idMigrationMap.get(unit.id)!,
                };

                // Also migrate schedule IDs if present
                if (
                  updatedUnit.schedule &&
                  Array.isArray(updatedUnit.schedule)
                ) {
                  updatedUnit.schedule = updatedUnit.schedule.map(
                    (schedule) => {
                      if (schedule.id && !isUUID(schedule.id)) {
                        ...
                        return {
                          ...schedule,
                          id: idMigrationMap.get(schedule.id)!,
                        };
                      }
                      return schedule;
                    },
