...
export const useNotificationsStore = create<NotificationsState>()(
  (set, get) => ({
      ...
      try {
        ...
        if (isAuthError) {
          ...
          if (!isNetworkError) {
            ...
            console.warn(
              "Notifications API unavailable; using local state fallback.",
            );
          }
    ...
    addNotification: async (notificationData) => {
      ...
      set((state) => ({
        notifications: [localNotification, ...state.notifications].slice(
          0,
          MAX_NOTIFICATIONS,
        ),
      }));
    ...
    markAsRead: async (id) => {
      ...
      try {
        ...
        errorHandler.logError(
          error instanceof Error
            ? error
            : new Error(`Failed to mark notification ${id} as read`),
    ...
    markAllAsRead: async () => {
      const originalReadStates = new Map(
    ...
    removeNotification: async (id) => {
      ...
      try {
        ...
        errorHandler.logError(
          error instanceof Error
            ? error
            : new Error(`Failed to remove notification ${id}`),
    ...
    clearAll: async () => {
      ...
      try {
        ...
        errorHandler.logError(
          error instanceof Error
            ? error
            : new Error("Failed to clear all notifications"),
