...
export async function GET(_request: NextRequest) {
  try {
    ...
    const allFactors: MFAFactor[] = (factorsData?.all ?? []).map(
      mapSupabaseFactor,
    );
    const verifiedFactors = allFactors.filter(
      (f: MFAFactor) => f.status === "verified",
    );
    const totpFactors = verifiedFactors.filter(
      (f: MFAFactor) => f.type === "totp",
    );
    const phoneFactors = verifiedFactors.filter(
      (f: MFAFactor) => f.type === "phone",
    );
