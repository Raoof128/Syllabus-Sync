...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { email, password, fullName, studentId, faculty, course, year } =
      parsed.data;
    ...
    if (devAutoConfirm && adminClient) {
      ...
      if (createError) {
        ...
        const isAlreadyRegistered = createError.message
          .toLowerCase()
          .includes("already");
    ...
    if (adminClient) {
      ...
      const { error: gamError } = await adminClient
