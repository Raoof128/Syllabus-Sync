...
export const mapUnitRow = (row: Row): Unit => {
  ...
  return {
    ...
    notificationEnabled: Boolean(
      row.notification_enabled ?? row.notificationEnabled ?? false,
    ),
...
export const mapEventRow = (row: Row): Event => {
  ...
  const startAt = parseDate(
    row.start_at ?? row.startAt ?? row.event_date ?? row.date,
  );
  ...
  return {
    ...
    sourcePublicEventId: row.source_public_event_id
      ? String(row.source_public_event_id)
      : null, // Track if from public feed
    title: String(row.title ?? ""),
...
export const serializeUnit = (
  unit: Omit<Unit, "schedule"> & { user_id?: string },
) => ({
  id: unit.id,
...
export const serializeDeadline = (
  deadline: Deadline & { user_id?: string },
) => ({
  id: deadline.id,
