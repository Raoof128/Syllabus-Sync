...
async function main() {
  const en = JSON.parse(
    await fs.readFile(path.join(LOCALES_DIR, BASE_LOCALE, TRANSLATION_FILE), 'utf8'),
  );
  const ar = JSON.parse(await fs.readFile(path.join(LOCALES_DIR, 'ar', TRANSLATION_FILE), 'utf8'));
