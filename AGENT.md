...
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/api/proxy.mfa.test.ts` ✅ (8 tests), `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs lib/proxy.ts tests/api/proxy.mfa.test.ts` ✅
Follow-ups: None — this completes the biometric login fix by closing the proxy gap left by the previous commit.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Biometric Login Full Audit — Align Settings Toggle, Login Status, and WebAuthn Credentials
Summary: Continued the biometric/passkey audit beyond the login button and found the settings-side biometric route was still metadata-only even after the login-path fix. Specifically, `app/api/auth/biometric/route.ts` reported `enabled` solely from legacy `auth.users.user_metadata.biometric_*` fields and disabling biometric cleared only metadata, leaving DB-backed `public.webauthn_credentials` intact. That meant settings could say biometric was disabled while DB-backed passkeys still existed, or show disabled even though valid passkeys were present. Hardened the full flow by making `GET /api/auth/biometric` treat DB-backed credentials as enabled and expose a credential count, and making `POST /api/auth/biometric` remove all user-owned `webauthn_credentials` when disabling biometric so the settings toggle now aligns with actual login capability. Existing login-path changes remain in place: login uses DB-backed WebAuthn auth endpoints and status checks both DB + legacy metadata.
Files Changed: `app/api/auth/biometric/route.ts`, `tests/api/auth/biometric.test.ts`
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/api/auth/passkey.status.test.ts tests/api/auth/biometric.test.ts tests/security/webauthn-auth-options.test.ts tests/security/webauthn-credentials.test.ts tests/unit/components/PasskeySecuritySection.test.tsx` ✅, `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs app/login/hooks/usePasskeyLogin.ts app/api/auth/passkey/status/route.ts app/api/auth/biometric/route.ts tests/api/auth/passkey.status.test.ts tests/api/auth/biometric.test.ts` ✅
Follow-ups: The codebase still carries duplicate concepts of “biometric enabled” vs “registered passkeys.” Functionally the flow is now aligned, but the long-term cleanup is to replace the legacy biometric toggle semantics with a single source of truth based on `webauthn_credentials` and simplify the settings UX around passkey presence rather than metadata flags.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Biometric Login Audit — Unify Login Availability with DB-Backed WebAuthn
Summary: Performed a focused end-to-end audit of biometric/passkey login and found the system was split across two parallel implementations. The security/settings UI registers passkeys through the newer DB-backed `/api/webauthn/*` flow into `public.webauthn_credentials`, but the login page still checked availability via `/api/auth/passkey/status` and authenticated via the legacy `/api/auth/passkey/*` endpoints that primarily rely on `auth.users.user_metadata.biometric_*`. That mismatch meant a user could successfully add a passkey in settings and still see biometric login as unavailable on the login screen. Fixed the active login path by switching `app/login/hooks/usePasskeyLogin.ts` to the DB-backed WebAuthn authentication endpoints and hardening `app/api/auth/passkey/status/route.ts` so availability is true when either a legacy metadata credential exists or at least one `webauthn_credentials` row exists for the user. Added regression coverage proving DB-backed credentials now surface as available in the login-status route.
Files Changed: `app/login/hooks/usePasskeyLogin.ts`, `app/api/auth/passkey/status/route.ts`, `tests/api/auth/passkey.status.test.ts`
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/api/auth/passkey.status.test.ts tests/security/webauthn-auth-options.test.ts tests/api/auth/biometric.test.ts` ✅, `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs app/login/hooks/usePasskeyLogin.ts app/api/auth/passkey/status/route.ts tests/api/auth/passkey.status.test.ts` ✅
Follow-ups: The repository still contains both legacy `user_metadata`-backed passkey endpoints and the newer `webauthn_credentials` implementation. They now interoperate for login availability/authentication, but the long-term cleanup is to retire the legacy `/api/auth/passkey/register*` + `/api/auth/biometric` metadata path and standardize the settings UI/status model on the DB-backed WebAuthn flow only.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Manage Profile Reminder Settings Persistence + Today Classes Passed State
Summary: Audited the reminder settings flow in `app/manage-profiles/components/ReminderSettings.tsx` and found the persistence path was optimistic but lossy: deadline/class/event toggles and timing selectors updated local Zustand state immediately, then fired a background `/api/user-preferences` write that swallowed failures and never rolled back. This meant the Manage Profile UI could appear saved even when the database update failed. Hardened `lib/store/notificationPreferencesStore.ts` so reminder-setting mutations now await API persistence, return success/failure, and rollback local state if the `user_preferences` write fails. Updated `ReminderSettings` to serialize reminder-setting saves, disable controls during save, and show a database error toast when persistence fails. Also improved `features/home/components/TodaySchedule.tsx` so past classes are visibly treated as passed by striking through the class code, title, time, and location while keeping the passed badge. Added route/store/UI regression coverage for reminder-setting persistence and the passed-class rendering path.
Files Changed: `lib/store/notificationPreferencesStore.ts`, `app/manage-profiles/components/ReminderSettings.tsx`, `features/home/components/TodaySchedule.tsx`, `locales/en/translations.json`, `tests/stores-notification-prefs.test.ts`, `tests/TodaySchedule.test.tsx`, `tests/api/user-preferences.route.test.ts`
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/stores-notification-prefs.test.ts tests/TodaySchedule.test.tsx tests/api/user-preferences.route.test.ts` ✅, `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs lib/store/notificationPreferencesStore.ts app/manage-profiles/components/ReminderSettings.tsx features/home/components/TodaySchedule.tsx tests/stores-notification-prefs.test.ts tests/TodaySchedule.test.tsx tests/api/user-preferences.route.test.ts` ✅
Follow-ups: A live authenticated browser test against a real Supabase session was not run in this pass. If you want end-to-end confirmation beyond route/store coverage, the next step is a Playwright run with a real login and network inspection for `/api/user-preferences`.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Custom Reminder Re-Arm Fix — Clear Stale notifiedAt on Edit
Summary: Investigated why custom-time reminders stopped firing even though the reminder row still existed. Root cause: editing an existing reminder reused the same reminder record and `updateReminder()` preserved its old `notifiedAt` timestamp. `useReminderChecker` skips any reminder with `notifiedAt`, so once a reminder had fired or been marked notified, changing it to a new custom time silently prevented all future alarms/toasts/bell notifications for that item. Fixed this centrally in `lib/store/remindersStore.ts` by re-arming reminders on schedule edits or re-enable actions, clearing `notifiedAt` whenever timing/custom date/custom time/item date/enabled state is updated. Added a regression test proving a notified reminder becomes pending again after being edited to a new custom time.
Files Changed: `lib/store/remindersStore.ts`, `tests/stores-misc.test.ts`
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/stores-misc.test.ts` ✅, `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs lib/store/remindersStore.ts tests/stores-misc.test.ts` ✅
Follow-ups: If custom reminders still fail after this patch, the next check is a live `useReminderChecker` integration trace to confirm whether the browser/tab lifecycle is suppressing timers rather than the reminder record itself.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Notification 409 Conflict Fix — System Reminder Bell Entries
Summary: Investigated the user-provided browser error `api/notifications 409` and found the real production root cause. A migration adds a unique index on `public.notifications(user_id, related_id, type)` when `related_id IS NOT NULL`, and `ReminderModal` was creating reminder-set/update/remove bell entries as `type: 'system'` with `relatedId: itemId`. Repeating reminder actions for the same item therefore hit the unique index, the API returned 409, and the store’s conflict path removed the optimistic temp notification, which looked like “appears then disappears.” Fixed this at the store layer by stripping `relatedId` from all `system` notifications before POST, because system bell entries are informational and should not participate in entity uniqueness. Also removed `relatedId` from `ReminderModal` bell notifications for clarity and added a regression test proving `system` notification POST bodies omit `relatedId`.
Files Changed: `lib/store/notificationsStore.ts`, `components/ui/ReminderModal.tsx`, `tests/stores.test.ts`
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/stores.test.ts` ✅, `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs lib/store/notificationsStore.ts components/ui/ReminderModal.tsx tests/stores.test.ts` ✅
Follow-ups: If any future notification category needs entity-bound uniqueness, it should use a non-`system` type or an explicit server upsert strategy instead of attaching `relatedId` to generic system bell entries.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Notification Disappears Follow-Up — Remove Auth-Driven Tear-Down on Focus
Summary: Investigated why the reminder bell item still disappeared after the earlier merge/store fixes. The remaining unstable path was auth-driven navigation, not notification-array replacement: `client-layout.tsx` still revalidated auth on every focus and could treat a transient focus-time null user as real sign-out, which unmounted the authenticated shell and could navigate toward `/login`, wiping the runtime and making the bell entry appear to vanish. Hardened `checkAuth()` to keep the authenticated shell mounted when a focus-time recheck returns null after a previously authenticated state, and moved real sign-out handling to explicit Supabase auth events (`SIGNED_OUT`, `SIGNED_IN`, `TOKEN_REFRESHED`) via `onAuthStateChange`. Also removed login-redirect side effects from `notificationsStore.loadNotifications()` auth-error handling so the store preserves state instead of owning navigation. Added a regression test proving 401 reloads preserve existing bell notifications.
Files Changed: `app/client-layout.tsx`, `lib/store/notificationsStore.ts`, `tests/stores.test.ts`
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/stores.test.ts tests/unit/utils/browserSession.test.ts` ✅, `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs app/client-layout.tsx lib/store/notificationsStore.ts lib/supabase/browserSession.ts tests/unit/utils/browserSession.test.ts tests/stores.test.ts` ✅
Follow-ups: If the bug still reproduces after this patch, the next step is live browser tracing of focus/auth/navigation events because the remaining candidate is an external full-page navigation or session reset, not the notifications store merge path.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Auth Focus Recovery Hardening for Notification Remount Regression
Summary: Hardened browser auth confirmation so protected routes do not tear down the authenticated shell on a single transient signed-out read during focus recovery or token refresh. Added `getConfirmedBrowserAuthSnapshot()` in `lib/supabase/browserSession.ts`, which retries once before callers treat the browser session as truly signed out. Wired this into `app/client-layout.tsx` so focus-triggered auth checks do not briefly flip `isAuthenticated` to false and remount `Header`, and into `lib/store/notificationsStore.ts` so auth-error fallback redirects only occur after a confirmed signed-out snapshot. Added regression tests covering resolved user, unknown resolution, transient null-then-user recovery, and confirmed signed-out behavior.
Files Changed: `lib/supabase/browserSession.ts`, `app/client-layout.tsx`, `lib/store/notificationsStore.ts`, `tests/unit/utils/browserSession.test.ts`
Verification: `npx vitest run --config config/vitest/vitest.config.ts tests/unit/utils/browserSession.test.ts` ✅, `npx tsc -p config/ts/tsconfig.json --noEmit` ✅, `npx eslint --config config/eslint/eslint.config.mjs app/client-layout.tsx lib/store/notificationsStore.ts lib/supabase/browserSession.ts tests/unit/utils/browserSession.test.ts` ✅
Follow-ups: Consider adding an integration test around `ClientLayout` focus handling if this auth path changes again.
...
Verification: `npm test` ✅ (91 files / 857 tests), `npx tsc --noEmit` ✅, `npx vercel --prod` ✅ (deployed)
Follow-ups: (1) Consider persisting the notifications store to localStorage as a cache for instant bell rendering on page load. (2) Add retry queue for failed notification POSTs.

Raouf: 2026-03-16 (Australia/Sydney)
Scope: Fix Reminder Alarm Firing — Widen Check Window, Background Tab Recovery, Fire Missed Reminders
Summary: Fixed user-set reminders (via ReminderModal) failing to fire their alarm. Three root causes: (1) `useReminderChecker` had a 5-minute fire window — browsers throttle `setInterval` in background tabs (once/min or suspended entirely), so the checker often missed the window and silently marked the reminder as notified without ever firing. Fix: widened window to 24 hours so missed reminders fire when the user returns. (2) Browser notification required both `permissionStatus === 'granted'` AND `pushEnabled` — but `pushEnabled` controls server-sent push, not local alarms the user explicitly set. Fix: fire browser notification whenever permission is granted, regardless of push toggle. (3) `reschedulePending()` silently discarded expired pending reminders on page reload instead of firing them. Fix: fire missed reminders up to 24 hours late on reload. Also reduced check interval from 30s to 15s and added `visibilitychange` listener for immediate check when tab regains focus.
Files Changed: `lib/hooks/useReminderChecker.ts`, `lib/store/notificationPreferencesStore.ts`
Verification: `npm test` ✅ (91 files / 857 tests), `npx tsc --noEmit` ✅, `npx vercel --prod` ✅ (deployed)
Follow-ups: (1) Consider syncing `remindersStore` to server so the cron job can also fire user-set custom reminders when the browser is closed. (2) Add integration test for `useReminderChecker` covering background-tab recovery scenario.

Raouf: 2026-03-15 (Australia/Sydney)
Scope: End-to-End Notification & Delete Audit — Cascade Notification Cleanup for Deadlines/Events
Summary: Full end-to-end trace of the notification system and all delete flows. Verified 6 prior fixes are intact (soft-delete consistency, clearAll 405, dark-mode icons, portal scroll-jump, modal propagation, temp-ID guard). Found 2 remaining gaps: `deadlinesStore.removeDeadline` and `eventsStore.removeEvent` did not clean up notifications referencing the deleted entity via `relatedId`, causing stale notification entries to persist in the bell dropdown. Fix: added notification cleanup via `useNotificationsStore.getState()` immediately after the optimistic delete (before the sync check / API call), so it works regardless of auth state. Added 2 regression tests confirming notification cleanup on deadline and event deletion.
Files Changed: `lib/store/deadlinesStore.ts`, `lib/store/eventsStore.ts`, `tests/stores.test.ts`
Verification: `npm run check` ✅ (secrets ✅, format ✅, typecheck ✅, lint ✅, 91 files / 857 tests ✅, build ✅)
Follow-ups: (1) `todosStore.removeTodo` could also benefit from notification cleanup if todos ever get relatedId notifications. (2) Delete confirmation modals in CalendarClient/FeedClient lack backdrop-click-to-close — intentional for mobile safety but worth revisiting for desktop UX.

Raouf: 2026-03-15 (Australia/Sydney)
Scope: Fix Notification Store Optimistic UI — Delete Guard + Rollback Order
Summary: Traced and fixed the `removeNotification` optimistic UI mismatch in `notificationsStore.ts`. Root cause: `removeNotification` sent `DELETE /api/notifications/temp-...` for optimistic (temp-prefixed) IDs, which always 404'd and was silently swallowed — the server-side notification survived and reappeared on next `loadNotifications`. Fix: added a UUID/temp-ID guard (matching the existing pattern in `markAsRead`) so temp or non-UUID IDs are removed locally only without an API call. Also fixed the rollback path: on non-404 API failure, the previous code appended the removed notification at the end of the array (visual jump); now it restores the full pre-delete snapshot to preserve original order. Added two regression tests: (1) removing a temp-ID notification makes no API call, (2) failed delete restores exact original order.
Files Changed: `lib/store/notificationsStore.ts`, `tests/stores.test.ts`
Verification: `npm run check` ✅ (secrets ✅, format ✅, typecheck ✅, lint ✅, 91 files / 855 tests ✅, build ✅)
Follow-ups: (1) If a user deletes a notification while its POST is still in-flight, the temp ID is removed locally but the server notification will appear on next load — consider tracking in-flight temp→server ID mappings if this becomes a real UX issue.

Raouf: 2026-03-15 (Australia/Sydney)
Scope: Notification & Delete Systems Audit — UUID Dedup, API Tests, Cascade Cleanup
Summary: Audited 8 reported bugs across notification and delete systems. Found 5 already fixed (event reminder tags, vitest config, dark mode buttons, ReminderModal propagation, deleteProfile ordering). Fixed 3 remaining issues: (1) Extracted duplicated `isValidUUID` from 4 files (notificationsStore ×2, deadlinesStore, todosStore, useFeedLogic) into shared `lib/utils/uuid.ts`. (2) Added 4 new notification API tests: POST validation (invalid body → 400), POST success (201), GET pagination metadata, GET page-2 offset calculation. (3) Added notification cleanup to `removeUnit` cascade — when a unit is deleted with `cascadeDeleted`, the code now captures affected deadline IDs before removal and cleans up related notifications from the notificationsStore. Also added a store-level integration test for cascade unit→deadline deletion.
Files Changed: `lib/utils/uuid.ts` (new), `lib/store/notificationsStore.ts`, `lib/store/deadlinesStore.ts`, `lib/store/todosStore.ts`, `lib/store/unitsStore.ts`, `features/feed/hooks/useFeedLogic.ts`, `tests/api/notifications.routes.test.ts`, `tests/stores.test.ts`
Verification: `npm run check` ✅ (secrets ✅, format ✅, typecheck ✅, lint ✅, 91 files / 853 tests ✅, build ✅)
Follow-ups: (1) Add server-side notification cleanup in `/api/units/[id]` DELETE handler for full consistency. (2) Add tests for `itemNotificationsStore` and `remindersStore` localStorage persistence. (3) Consider syncing `remindersStore` to server for cross-device reminder support.

Raouf: 2026-03-14 (Australia/Sydney)
Scope: Full Production Readiness Audit — Fix All Critical/Warning Findings
Summary: Comprehensive production readiness audit across 7 domains (config, API security, pages/components, lib/services, CI/CD, tests, security). Fixed 14 critical and 20 warning findings. Security: patched IDOR in `/api/sync` (added `user_id` scoping to UPDATE/DELETE), added `/api/cron/` and `/api/security/rate-limit/cleanup` to proxy public path allowlist (unblocking cron jobs), replaced `getSession()` with `getUser()` in `lib/supabase/middleware.ts` for server-side JWT validation, fixed offline sync queue to include credentials and CSRF token, added body size limit to `/api/audit` POST, added CSRF origin validation to `/api/navigate`, refactored `/api/profiles` and `/api/user-preferences` to use `requireAuth`/`requireAuthWithRateLimit` for consistent CSRF validation, fixed error message leak in profiles PUT, added audit log for `_forceVersion` usage. Infrastructure: fixed Sentry enablement condition (gate on DSN not AUTH_TOKEN), fixed Sentry environment tagging to use `VERCEL_ENV`, changed CORP header from `same-site` to `cross-origin`, added CI build env vars, populated branch protection contexts, added Google API key and VAPID patterns to secrets scanner. IP anomaly detection: added production guard to prevent plaintext HTTP geolocation calls, added explicit stub warnings. UI/UX: converted 5 settings pages from client to server components for metadata export, added `error.tsx` for calendar/map/feed routes, added `loading.tsx` for settings, fixed SSR guard on FeedClient portal, added deleteProfile rollback on API failure. Quality: moved `@types/*` to devDependencies. All tests pass (848/848), build clean, typecheck clean.
Files Changed: `app/api/sync/route.ts`, `lib/proxy.ts`, `lib/supabase/middleware.ts`, `lib/store/offlineSyncStore.ts`, `app/api/audit/route.ts`, `app/api/navigate/route.ts`, `app/api/profiles/route.ts`, `app/api/user-preferences/route.ts`, `lib/security/ip-anomaly-detection.ts`, `config/next/next.config.ts`, `config/sentry/sentry.client.config.ts`, `config/sentry/sentry.server.config.ts`, `config/sentry/sentry.edge.config.ts`, `app/feed/FeedClient.tsx`, `lib/store/profilesStore.ts`, `app/settings/security/page.tsx`, `app/settings/security/PrivacySettingsPage.tsx`, `app/settings/general/page.tsx`, `app/settings/general/GeneralSettingsContent.tsx`, `app/settings/appearance/page.tsx`, `app/settings/appearance/AppearanceSettingsContent.tsx`, `app/settings/experience/page.tsx`, `app/settings/experience/ExperienceSettingsContent.tsx`, `app/settings/about/page.tsx`, `app/settings/about/AboutSettingsContent.tsx`, `app/calendar/error.tsx`, `app/map/error.tsx`, `app/feed/error.tsx`, `app/settings/loading.tsx`, `package.json`, `.github/workflows/ci-cd.yml`, `.github/community.yml`, `tools/security/check-secrets.mjs`, `tests/api/profiles.route.test.ts`, `tests/stores-profiles.test.ts`
Verification: `npx tsc --noEmit` ✅; `npm run test` ✅ (91 files, 848 tests); `npm run build` ✅
Follow-ups: (1) Set `CRON_SECRET` in Vercel production env. (2) Remove `ALLOW_MEMORY_RATE_LIMIT=true` from Vercel prod and configure Upstash Redis. (3) Fix trailing `\n` in `WEBAUTHN_ORIGIN`/`WEBAUTHN_RP_ID` in Vercel env. (4) Apply branch protection rules via GitHub repo settings UI (community.yml is documentation only). (5) Write tests for untested API routes (sync, todos, deadlines, events, cron).
...
Verification: `rg -n "production-deploy\\.yml" README.md docs .github -g '!AGENT.md' -g '!CHANGELOG.md'` returned no remaining repo-doc references; `git diff` confirms the workflow removal plus documentation alignment
Follow-ups: If a `Vercel - ...` GitHub status still appears, remove or disable Git checks from the Vercel project integration in the Vercel dashboard; that external status is not defined by repository workflow files.
...
Files Changed: `.github/workflows/ci-cd.yml`, `.github/workflows/production-deploy.yml`, `tests/map/useMapLocation.test.ts`
Verification: reproduced failing test via `npm run test:coverage` (1 failing assertion in `tests/map/useMapLocation.test.ts`); reproduced security failure via `npm audit --audit-level high` (dev-only `vercel` CLI dependency tree); confirmed `NODE_ENV=production npm run build` ✅ before workflow changes
Follow-ups: Rerun GitHub Actions on the new commit. If Vercel’s external status check is still red after this, that will indicate a deployment-side/project-integrations issue rather than repo CI logic.
...
Verification: `supabase db push --yes` ⚠️ advanced past `20260108131028` and surfaced the not-null failure in shared-event seeding; migration patched to skip that seed when incompatible
Follow-ups: Rerun `supabase db push --yes` and continue resolving any remaining legacy replay incompatibilities until the remote reaches the latest migration.

Raouf: 2026-03-13 (Australia/Sydney)
Scope: Supabase Migration Replay Compatibility
Summary: A second `supabase db push --yes` attempt showed the same `units_user_code_unique` replay failure, but the server surfaced it as SQLSTATE `42P07` on the backing relation name rather than `duplicate_object`. Broadened the exception handling inside `20260108131028_add_user_id_and_rls_policies.sql` to absorb both `duplicate_object` and explicit `42P07`, which is the same legacy-idempotency condition under a different PostgreSQL error classification. This keeps the migration safe on clean databases while allowing replay to proceed on drifted remotes that already have the unique index/constraint artifacts.
Files Changed: `supabase/migrations/20260108131028_add_user_id_and_rls_policies.sql`
Verification: `supabase db push --yes` ⚠️ reproduced SQLSTATE `42P07`; migration updated to handle that replay case
Follow-ups: Rerun `supabase db push --yes` and continue clearing any remaining historical migration conflicts.

Raouf: 2026-03-13 (Australia/Sydney)
Scope: Supabase Legacy Migration Idempotency Hardening
Summary: The first rerun of `supabase db push --yes` still failed on `20260108131028_add_user_id_and_rls_policies.sql` because PostgreSQL raised `duplicate_object` for `units_user_code_unique` even after adding an existence check by constraint definition. This indicates the drifted remote schema exposes the duplicate through object state that is not reliably discoverable by the catalog predicate alone during replay. Hardened the migration again by wrapping the `ALTER TABLE ... ADD CONSTRAINT units_user_code_unique` statement in a `duplicate_object` exception block inside the guarded `DO $$` section, making the migration safely idempotent for drifted or partially-provisioned environments while preserving the intended constraint on clean databases.
Files Changed: `supabase/migrations/20260108131028_add_user_id_and_rls_policies.sql`
Verification: `supabase db push --yes` ⚠️ reproduced duplicate-object failure against remote; migration patched to absorb that exact duplicate case on replay
Follow-ups: Rerun `supabase db push --yes` and continue addressing any additional non-idempotent legacy migrations until the remote is fully aligned.

Raouf: 2026-03-13 (Australia/Sydney)
Scope: Supabase Remote Migration Repair + Push Sync
Summary: Investigated the production Supabase migration state after deployment and found the remote migration history contained an orphan `20260312` entry plus only the first two local migrations recorded, while the underlying schema already included later objects. Repaired the remote history entry as reverted using the Supabase CLI, attempted a full `supabase db push`, and identified the first hard failure in `20260108131028_add_user_id_and_rls_policies.sql`: the migration only guarded on the constraint name `units_user_code_unique`, but the remote database already had an equivalent `UNIQUE (user_id, code)` constraint under existing drifted state. Hardened the migration to detect equivalent unique constraints by table/schema/definition as well as by name so future `supabase db push` runs can progress against partially-provisioned environments instead of failing on this duplicate-constraint edge case.
Files Changed: `supabase/migrations/20260108131028_add_user_id_and_rls_policies.sql`
Verification: `supabase migration repair --status reverted 20260312` ✅; `supabase migration list` ✅; `supabase db push` ⚠️ reached remote and surfaced the duplicate-constraint failure now handled by this migration patch
Follow-ups: Rerun `supabase db push` after this patch and continue resolving any additional legacy migration idempotency issues until the remote migration history reaches `20260313093000`.

Raouf: 2026-03-13 (Australia/Sydney)
Scope: Push Notifications Audit + Web Push Delivery Repair
Summary: Completed a full push-notification audit and fixed the root architecture break. Findings: the app claimed to support push notifications but only had client-side `setTimeout` reminders plus `Notification`/`showNotification` calls, so reminders died when the tab/browser closed; the service worker had no `push` or `notificationclick` handlers, so service-worker notifications could not reliably deep-link on click; reminder timings and per-type toggles lived only in local Zustand/localStorage, so the server had no data to deliver reminders in the background; and there was no Push API subscription storage or cron sender at all. Implemented real web-push plumbing: added VAPID env support and `web-push`, created durable Supabase schema for push subscriptions + reminder delivery dedupe plus server-side reminder preference columns, added authenticated subscription management route (`/api/push/subscription`), added cron-backed reminder sender (`/api/cron/push-reminders`) for deadline/event reminders, upgraded the service worker with `push` + `notificationclick` handling, and updated the notification service/store/settings flows so enabling notifications now subscribes the browser and syncs push/reminder preferences to the server while preserving the existing in-session scheduler for immediate/local reminders.
Files Changed: `package.json`, `package-lock.json`, `.env.example`, `vercel.json`, `supabase/migrations/20260313093000_add_web_push_infrastructure.sql`, `app/api/push/subscription/route.ts`, `app/api/cron/push-reminders/route.ts`, `app/api/user-preferences/route.ts`, `app/api/types.ts`, `lib/server/push.ts`, `lib/services/notificationService.ts`, `lib/store/notificationPreferencesStore.ts`, `lib/supabase/database.types.ts`, `public/sw.js`, `features/settings/components/NotificationSettings.tsx`, `app/manage-profiles/components/ReminderSettings.tsx`, `tests/api/push.subscription.route.test.ts`, `tests/stores-notification-prefs.test.ts`, `tests/settings/NotificationSettings.test.tsx`
Verification: `npm run typecheck` ✅; `npx eslint --config config/eslint/eslint.config.mjs app/api/push/subscription/route.ts app/api/cron/push-reminders/route.ts app/api/user-preferences/route.ts app/manage-profiles/components/ReminderSettings.tsx features/settings/components/NotificationSettings.tsx lib/server/push.ts lib/services/notificationService.ts lib/store/notificationPreferencesStore.ts tests/api/push.subscription.route.test.ts tests/stores-notification-prefs.test.ts tests/settings/NotificationSettings.test.tsx` ✅; `npm run test -- tests/stores-notification-prefs.test.ts tests/settings/NotificationSettings.test.tsx tests/api/push.subscription.route.test.ts` ✅ (45/45)
Follow-ups: Background web push is now real for deadline/event reminders once VAPID keys and cron auth are configured in the environment. Class reminders still retain the existing client-side scheduler path; a fully server-side class push sender should wait for an explicit per-user timezone model because class schedules are stored as weekly local-time tuples rather than absolute timestamps.

Raouf: 2026-03-12 (Australia/Sydney)
Scope: Map UI — Align Google search bar with campus map position
Summary: Removed the extra vertical offset on the Google-mode building search bar so the shared `map-search-input` now appears in the same top-left screen position as the campus map search UI. Updated `CampusMapHUD.tsx` from `top-14` to `top-3` and corrected the nearby comment so the two map modes stay visually aligned when switching views.
...
Follow-ups: Visually confirm on desktop and mobile that the Google search bar clears any native map chrome at the top edge.

Raouf: 2026-03-12 (Australia/Sydney)
Scope: Map Tooling — Export calibrated Flutter overlay metadata
Summary: Extended the Flutter map export bridge so it now ships the same calibrated campus overlay metadata the web map depends on, not just the raster image and building registry. `tools/export_buildings.mjs` now derives pixel bounds, fit-padding / zoom behavior, and affine GPS projection coefficients from the web map calibration source and writes them into Flutter’s `campus_overlay_meta.json`. This keeps Flutter aligned with the web `CRS.Simple` campus map for image bounds, GPS-to-overlay route projection, and marker offset behavior without copying the web component code itself.
Files Changed: `tools/export_buildings.mjs`
Verification: `node --experimental-strip-types tools/export_buildings.mjs` ✅ (153 buildings exported; calibrated overlay metadata + raster copied to Flutter repo)
...
Verification: `npx tsc --noEmit` ✅; lint ✅; prettier ✅; `npm run build` ✅

Raouf: 2026-03-12 (Australia/Sydney)
Scope: Map UI — Full Bug-Fix Pass (8 Issues)
Summary: Comprehensive map UX audit fixing 8 issues across both Campus Map and Google Maps modes. (1) Duplicate Share/Export: Removed the floating Share+Export toolbar from CampusMapHUD campus mode — MapClient's toolbar is now the single canonical source for both actions. (2) My Location overlap: Moved Locate Me / My Location buttons from `right-[10px]` to `left-3` on both CampusMap and GoogleMapCanvas, avoiding collision with Google native controls (zoom, Pegman, mapType) and Leaflet zoom controls. CampusMap button shifts to `bottom-[220px]` on mobile when a building card is visible. GoogleMapCanvas transitions between `bottom-20` and `bottom-[28rem]` based on route panel visibility. (3) Search bar mobile: Campus search bar width increased from 360px to 400px (matches Google mode). Removal of the competing top-right toolbar eliminates overlap on narrow screens. (4) Street View controls: Added `visible_changed` listener on `map.getStreetView()` in GoogleMapCanvas. Street View state flows up through GoogleMapController to MapClient. When Street View is active: My Location button hides, CampusMapHUD returns null, route panel and compact nav bar hide. (5) Dark mode contrast: Migrated GoogleRoutePanel from shadcn vars (`bg-background`, `text-foreground`, `text-muted-foreground`, `bg-muted`, `border-border`) to project `mq-*` design tokens (`bg-mq-card-background`, `text-mq-content`, `text-mq-content-secondary`, `bg-mq-background-secondary`, `border-mq-border`). Same migration for GoogleMapController's compact nav bar. Stop buttons now use explicit `bg-red-600 text-white dark:bg-red-500`. (6) Text readability: All text in route panel, step list, travel mode tabs, and compact nav bar now uses `mq-*` tokens with proper dark mode contrast. Step numbers use `bg-mq-background-secondary text-mq-content`. Error text uses `text-red-600 dark:text-red-400`. (7) Control consistency: Both Locate Me buttons now share identical styling: `rounded bg-white shadow-[0_1px_4px_rgba(0,0,0,0.3)] hover:bg-gray-100 active:bg-gray-200`, h-10 w-10, matching crosshair SVG with consistent line coordinates (2→6, 18→22). (8) Mobile polish: CampusMap button at `bottom-[220px] sm:bottom-12` / `bottom-12`, preventing the button from overlapping the building info card on mobile.
...
Verification: `npx tsc --noEmit` ✅; `npx eslint` on all 6 files ✅; `npx prettier --check` ✅; `npm run build` ✅; map tests 9/11 passed (2 pre-existing i18n test failures)
Follow-ups: The 2 pre-existing i18n test failures in `RouteAnnouncer.test.tsx` and `useMapNavigation.test.ts` need their hardcoded English assertions updated to translation keys.
...
Follow-ups: Route panel text could be i18n-ised. Voice/haptic navigation feedback could be wired from realtimeNavigation.ts. Marker clustering for low-zoom levels would improve performance.

Raouf: 2026-03-09 (Australia/Sydney)
Scope: Google Maps Clean Slate — Simplified JS API Stack
Summary: Replaced the Google Maps component stack with a clean, self-contained architecture. Rewrote `GoogleMapCanvas` (removed street view, heading indicator, accuracy circle; simplified building/user/external markers), `GoogleMapController` (removed `forwardRef`/`useImperativeHandle`; now manages its own travel mode, navigation, and route state internally instead of deferring to parent), and `GoogleRoutePanel` (self-contained panel with travel mode tabs, route info, start/stop navigation, and Google Maps handoff link). Cleaned up `lib/maps/google/types.ts` (removed dead types `GoogleMapProvider`, `SelectedDestination`, `GoogleRouteState`; added `ExternalDestination` type), simplified `lib/maps/google/loader.ts` (clean singleton with stale script cleanup), and added `https://places.googleapis.com` to CSP `connect-src`. Simplified `MapClient.tsx` Google mode block by removing the `forwardRef` pattern, `googleMapRef`, `googleTravelMode` state, `handleSelectBuilding` callback, and nav state wiring — the Controller now handles all Google Map state internally. API routes (`routes`, `place-search`, `place-details`) kept unchanged. Campus map files untouched.
...
Verification: `npx tsc --noEmit` ✅; `npx eslint --config config/eslint/eslint.config.mjs` on all changed files ✅; `npm run build` ✅; `npm run test -- tests/map` 10/12 passed (2 pre-existing i18n test failures in `RouteAnnouncer.test.tsx` and `useMapNavigation.test.ts`)
Follow-ups: The 2 pre-existing test failures need their hardcoded English assertions updated to translation keys. Building marker click-to-select could be re-added as an optional `onSelectBuilding` prop if needed. The route panel could be i18n-ised in a follow-up pass.

Raouf: 2026-03-08 (Australia/Sydney)
Scope: Home FAB Quick-Add Audit Hardening — Sidebar Scroll Support and Overlay Reset
Summary: Performed a follow-up audit on the new state-driven Home FAB → Calendar intent flow and fixed the two remaining issues discovered in review. First, the desktop widget sidebar could still leave lower widgets below the fold with no reliable scroll ancestor for the intent hook to control, so `features/calendar/components/CalendarSidebar.tsx` now provides a bounded desktop height with `overflow-y-auto`, giving the orchestration hook a stable nested scroll container for assignment/exam/event/reminder widget targeting. Second, the intent flow only checked whether the target add dialog was already open and could stack on top of another open detail panel or confirm modal, so `app/calendar/CalendarClient.tsx` now exposes a single `prepareForPendingIntent` callback that closes all calendar add dialogs, detail panels, and delete-confirm overlays before the requested add form is opened. The orchestration hook now calls that reset callback immediately before opening the target form. This closes the remaining modal-conflict and desktop-sidebar sequencing gaps from the first audit pass.
...
Follow-ups: The quick-add orchestration path is now structurally sound. The next improvement should be an end-to-end browser test that asserts the full FAB → widget → modal flow on both desktop and mobile breakpoints.

Raouf: 2026-03-08 (Australia/Sydney)
Scope: Home FAB Quick-Add Orchestration Fix — State-Driven Calendar Intent Flow
Summary: Replaced the Home page floating quick-action FAB’s brittle query-param flow with a state-driven calendar intent pipeline. The FAB in `app/home/HomeClient.tsx` now only queues a pending calendar intent (`target`, `highlight`, `autoOpenForm`, `requestedAt`, `requestId`) via a new Zustand store and navigates to `/calendar`, instead of trying to force immediate URL-driven widget/action sequencing. Added `features/calendar/hooks/usePendingCalendarIntent.ts`, which runs on the calendar route after hydration, waits for the target widget anchor to mount, detects the correct scroll container, skips scrolling when the widget is already visible, applies a temporary red ring/glow highlight, opens the mapped form only after the widget is ready, and clears the intent once execution succeeds. Added stable widget anchors (`calendar-widget-unit`, `calendar-widget-assignment`, `calendar-widget-exam`, `calendar-widget-event`, `calendar-widget-reminder`) and ref wiring through `CalendarClient`/`CalendarWidgets` so the orchestration works against stable refs instead of fragile selectors or URL timers. The old FAB `highlightWidget + action` timer chain in `CalendarWidgets` was removed because it was the root of the race between route changes, DOM mount timing, and modal state.
Files Changed: `app/home/HomeClient.tsx`, `app/calendar/CalendarClient.tsx`, `features/calendar/components/CalendarWidgets.tsx`, `features/calendar/components/widgets/AssignmentsWidget.tsx`, `features/calendar/components/widgets/ExamsWidget.tsx`, `features/calendar/components/widgets/EventsWidget.tsx`, `features/calendar/components/widgets/TodosWidget.tsx`, `features/calendar/components/widgets/UnitsWidget.tsx`, `features/calendar/hooks/usePendingCalendarIntent.ts` (NEW), `features/calendar/lib/calendarIntent.ts` (NEW), `lib/store/calendarIntentStore.ts` (NEW)
Verification: `npx eslint --config config/eslint/eslint.config.mjs app/home/HomeClient.tsx app/calendar/CalendarClient.tsx features/calendar/components/CalendarWidgets.tsx features/calendar/components/widgets/AssignmentsWidget.tsx features/calendar/components/widgets/ExamsWidget.tsx features/calendar/components/widgets/EventsWidget.tsx features/calendar/components/widgets/TodosWidget.tsx features/calendar/components/widgets/UnitsWidget.tsx features/calendar/hooks/usePendingCalendarIntent.ts features/calendar/lib/calendarIntent.ts lib/store/calendarIntentStore.ts` ✅; `npm run typecheck` ✅
Follow-ups: If this quick-add pattern is later reused from pages other than Home, the same store/hook path can be reused without adding more route-query orchestration. Existing deep-link highlight params (`highlightUnit`, `highlightDeadline`, `highlightEvent`, `highlightTodo`, `section`) remain intact and separate from the new quick-add intent flow.
...
Summary: Completed a repo-first internationalisation audit across the Next.js frontend and map proxy stack. Localised the Google Maps route/place APIs by threading selected locale through client hooks and server proxies, migrated remaining production user-facing hardcoded strings in map/profile/reminder/auth/contact flows to translation keys, added the required English source keys, repaired locale structural drift (missing keys, extra stray key, placeholder mismatches), and regenerated non-English locale coverage while preserving existing key structure.
...
Verification: `node tools/i18n/deep-audit.mjs` ✅ (34/34 non-English locales at full key coverage, 0 missing/extra/empty/placeholder mismatches), `npm run check:i18n` ✅, `npm run typecheck` ✅, `npm run lint` ✅. Remaining hardcoded-string scan results are limited to dev-only UI labels and literal example placeholders (`MFAChallenge`, `PersonalInfoCard`, `SMSSetup`, `DevPinPanel`, `LiquidGlassCard`).

Raouf: 2026-03-13 (Australia/Sydney)
Scope: Documentation Reconciliation — README, Architecture, API, Setup, Inventory, Navigation
Summary: Performed a code-backed documentation reconciliation pass against the current app bootstrap, route tree, settings/navigation structure, package scripts, API handlers, Supabase migration layout, and CI workflows. Replaced stale README/setup/API/deployment content, added missing canonical docs for technical explanation, environment setup, repository inventory, and route/navigation behavior, created compatibility entry points for `README_Syllubus_Sync..md` and `docs/ARCHITECTURE.md`, and updated operational docs to reflect the live Google Maps proxy usage, cron endpoints, Vercel env validation, and hidden env toggles used by CSRF, Sentry, rate limiting, geofencing, and the admin map API.
Files Changed: `README.md`, `README_Syllubus_Sync..md`, `TECHNICAL_EXPLANATION.md`, `docs/ARCHITECTURE.md`, `docs/README.md`, `docs/architecture/ARCHITECTURE.md`, `docs/api/API_REFERENCE.md`, `docs/operations/deployment-checklist.md`, `docs/operations/ENVIRONMENT_SETUP.md`, `docs/operations/google-maps-platform-setup.md`, `docs/operations/resend-vercel-setup.md`, `docs/reference/REPOSITORY_INVENTORY.md`, `docs/reference/ROUTES_AND_NAVIGATION.md`, `supabase/README.md`, `.env.example`, `AGENT.md`, `CHANGELOG.md`
Verification: `npx prettier --check README.md README_Syllubus_Sync..md TECHNICAL_EXPLANATION.md docs/ARCHITECTURE.md docs/README.md docs/architecture/ARCHITECTURE.md docs/api/API_REFERENCE.md docs/operations/deployment-checklist.md docs/operations/ENVIRONMENT_SETUP.md docs/operations/google-maps-platform-setup.md docs/operations/resend-vercel-setup.md docs/reference/REPOSITORY_INVENTORY.md docs/reference/ROUTES_AND_NAVIGATION.md supabase/README.md` ✅; custom Markdown link existence check across the modified docs ✅. No TypeScript/runtime code changed in this pass, so app build/test commands were not rerun.
Follow-ups: Consider adding an automated docs drift check that snapshots `app/api/**/route.ts` methods and `app/**/page.tsx` routes so future README/API/navigation docs can be diffed in CI.

Raouf: 2026-03-14 (Australia/Sydney)
Scope: Documentation Link Redirect — GitHub Doc Target to MQ Website
Summary: Applied the requested narrow documentation-link correction only. Updated the shared `EXTERNAL_LINKS.documentation` value in `lib/config.ts` from the prior GitHub target to `https://www.mq.edu.au/`, which redirects the app’s About page, Contact page, and Settings -> About documentation/help actions to the Macquarie University website. Left all non-documentation GitHub links, repository metadata, security-reporting endpoints, clone instructions, and third-party service URLs unchanged.
Files Changed: `lib/config.ts`, `AGENT.md`, `CHANGELOG.md`
Verification: Repo search confirmed the app’s live documentation/help actions resolve through `EXTERNAL_LINKS.documentation`, so this single-source change updates all current documentation buttons/links without affecting unrelated URLs.
Follow-ups: None.
