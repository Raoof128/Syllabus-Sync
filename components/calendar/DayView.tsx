...
export default function DayView({
    ...
    if (isToday && currentHour >= START_HOUR && currentHour < START_HOUR + HOURS.length) {
        currentTimePosition = (currentHour - START_HOUR) * HOUR_HEIGHT + (currentMinute / 60) * HOUR_HEIGHT;
    }
                ...
                <div className="relative" style={{ height: HOURS.length * HOUR_HEIGHT + 24 }}>
                    ...
                    {HOURS.map((hour, index) => (
                        ...
                        >
                            ...
                            <div className={cn("flex-1 border-t border-mq-border/50", isToday && "bg-mq-primary/5")} style={{ height: HOUR_HEIGHT }} />
                        </div>
                    ))}
                                ...
                                >
                                    <div className="p-1 h-full overflow-hidden">
                                        ...
                                        <span className="text-[10px] text-mq-content-secondary block">
                                            {formatScheduleTime(schedule.startTime, 'en')} - {formatScheduleTime(schedule.endTime, 'en')}
                                        </span>
                                        <span className="text-[10px] text-mq-content-tertiary block truncate mt-0.5">
                                            {formatLocation(unit.location.building, unit.location.room, t('room' as TranslationKey))}
                                        </span>
                                    </div>
                                </button>
                            );
