...
async function checkRLSDetail() {
  ...
  console.log('\n📋 CUSTOM USERS TABLE:');
  const { data: customUsers, error: usersErr } = await supabase.from('users').select('*').limit(3);

  if (usersErr) {
    console.log(
      `   ${usersErr.message.includes('does not exist') ? '(Table does not exist - using auth.users)' : usersErr.message}`,
    );
  } else if (customUsers) {
