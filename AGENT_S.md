...
Scope: Notification Accuracy, Profile Reload Speed, Auth Flow Hardening
Summary: Three-area fix covering notification message accuracy, profile store performance, and auth flow hardening. Notifications: replaced `Date.now()` in class reminder tag with stable `classTimestamp` to fix duplicate notification spam (notificationService.ts:449), improved cron push payloads with real titles/due dates instead of generic "Event Reminder"/"is due soon", added `relatedId` to push data payload enabling Header overdue badge detection. Profiles: added SWR-like caching to profilesStore with `lastFetched` timestamp and 60s TTL (skip refetch if fresh, unless `force: true`), added `Cache-Control: private, max-age=0, must-revalidate` to profiles GET endpoint, added force refresh after ReminderSettings preference toggle. Auth: added `email_confirmed_at` gate in proxy middleware redirecting unverified users to `/verify?reason=unverified` on protected routes, added explicit `email_confirmed_at` check in login action (no longer relies on Supabase error string parsing), created Postgres trigger `on_auth_user_created` for database-level signup atomicity (eliminates orphaned auth users), replaced 800ms setTimeout redirect with `onAuthStateChange` listener for event-driven session mounting, fixed raw Supabase error leak in password reset page. All tests pass (848+), build clean, typecheck clean. Postgres trigger migration applied to production. Deployed to Vercel.
Files Changed: `lib/services/notificationService.ts`, `lib/store/notificationPreferencesStore.ts`, `app/api/cron/push-reminders/route.ts`, `lib/store/profilesStore.ts`, `app/api/profiles/route.ts`, `app/manage-profiles/components/ReminderSettings.tsx`, `lib/proxy.ts`, `app/login/actions.ts`, `app/api/auth/signup/route.ts`, `app/login/LoginClient.tsx`, `app/reset-password/reset-password-client.tsx`, `supabase/migrations/20260314_auto_create_profile_trigger.sql`, `tests/api/proxy.mfa.test.ts`
Verification: `npm run check` ✅ (secrets, format, typecheck, lint, test, build); `supabase db push` ✅; `npx vercel --prod` ✅
Follow-ups: (1) Monitor push notification delivery with real titles in production. (2) Verify email verification gate doesn't block OAuth callback flows. (3) Consider adding class reminders to cron push system (currently client-side only due to weekly local-time schedule format).
